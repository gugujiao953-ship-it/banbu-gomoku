...
import { downloadText, exportJson, exportPos, exportSgf, importRecordFile, mainLineLength } from "./formats";
...
import type { BoardMark, BoardMarkStyle, GameDocument, ImportResult, NodeEvaluation, Position, RecordNode } from "./types";
...
export default function App() {
  ...
  useEffect(() => () => { vcfWorker.current?.terminate(); puzzleAiWorker.current?.terminate(); pagedSession.current?.close(); dynamicViewSession.current?.close(); }, []);
  useEffect(() => {
  ...
  const recordDraft = (operation: Parameters<typeof pushDraft>[1]) => {
    if (isPagedLibraryView(document) || isDynamicDatabaseView(document)) { setToast("当前数据库棋谱仅支持保真浏览，不原地编辑"); return; }
  ...
  const play = (position: Position) => {
    ...
    if (board[position.row][position.col]) {
      ...
      if (node && dynamicViewSession.current && isDynamicDatabaseView(document)) {
        const depth = path.findIndex((item) => item.id === node.id);
        void dynamicViewSession.current.toDepth(depth).then((opened) => { setDocument(opened.document); setCurrentId(opened.currentId); });
      } else if (node) setCurrentId(node.id);
      setCandidateLabel(null);
      return;
    }
    if (mode === "record" && dynamicViewSession.current && isDynamicDatabaseView(document)) {
      const target = current.children.map((id) => viewDocument.nodes[id]).find((node) => node?.move?.row === position.row && node.move.col === position.col);
      if (!target) { setToast("这个位置不在当前数据库分支中"); return; }
      void dynamicViewSession.current.move(position).then((opened) => { setDocument(opened.document); setCurrentId(opened.currentId); }).catch(() => setToast("数据库分支读取失败"));
      return;
    }
    if (mode === "record" && isPagedLibraryView(document)) {
      setCandidateLabel(null); setEditMoveMode(false);
      setToast("当前分页大谱仅支持保真浏览，不原地编辑");
      return;
    }
  ...
  const chooseChild = (id: string, pivotId = currentId) => {
    if (dynamicViewSession.current && isDynamicDatabaseView(document)) {
      ...
      if (node?.move) void dynamicViewSession.current.move(node.move).then((opened) => { setDocument(opened.document); setCurrentId(opened.currentId); setSheet(null); });
      return;
    }
    const session = pagedSession.current;
    if (session) {
  ...
  const deleteCurrentVariation = () => {
    if (isPagedLibraryView(document) || isDynamicDatabaseView(document)) { setToast("当前数据库棋谱仅支持保真浏览，不原地删除"); return; }
  ...
  const handleFiles = async (files?: FileList | File[]) => {
    ...
    if (singleExtension === "lib") {
      ...
      try {
        ...
        openRecord(opened.document, opened.currentId, { onOpened: () => setImportState("renlib-web-query-ready", { name: requested[0].name, size: requested[0].size }) });
    ...
    if (singleExtension === "db" || singleExtension === "dp") {
      ...
      try {
        ...
        openRecord(opened.document, opened.currentId, { onOpened: () => setImportState("dp-query-ready", { records: opened.recordCount }) });
  ...
  return <div className="app-shell">
    ...
    <main className="app-main">
      {tab === "record" && <div className="record-page">
        ...
        <Board document={viewDocument} currentId={currentId} showNumbers={showNumbers} showCoordinates={showCoordinates} largeBoard={largeBoard} rotation={rotation} mirrored={mirrored} initialDepth={mode === "puzzle" ? puzzleInitialDepth : 0} disabled={(mode === "puzzle" && (aiThinking || !!puzzleOutcome)) || isPagedLibraryView(document)} onPlay={play} onMark={mode === "record" && !isDynamicDatabaseView(document) ? mark : () => undefined}/>
        <div className={`workspace-status ${puzzleOutcome || ""}`}>{mode === "record" ? <><div className="workspace-status-copy"><span>{hasDraft(draft) ? "有未保存草稿" : candidateLabel ? `点棋盘放置标注「${candidateLabel}」` : current.move ? `${current.move.player === "black" ? "黑" : "白"} · ${coordinateName(current.move)}` : nodeKindLabel(current)}</span><small>{depthOf(viewDocument, currentId)} / {compactNodeCount(document) ? "大型" : mainLineLength(document)} 手 · {branchCount(viewDocument)} 处分支</small></div><div className="record-command-bar" aria-label="常驻打谱工具">
            ...
            <button className="command-delete" onClick={deleteCurrentVariation} disabled={!current.parentId || isPagedLibraryView(document) || isDynamicDatabaseView(document)} aria-label="删除当前一步及后续变化" title={isPagedLibraryView(document) || isDynamicDatabaseView(document) ? "数据库基线为只读视图" : current.parentId ? "删除本步及后续变化" : "起始局面不可删除"}><Trash2/></button>
...
{mode === "puzzle" && dockPanel === "puzzles" && <><button onClick={() => movePuzzle(-1)}><ChevronLeft/><span>上一题</span></button><button className="accent" onClick={() => { setWorkspaceSelectorOpen(true); setWorkspaceListExpanded(true); setExpandedCollectionId(puzzleCollections[puzzleCollectionIndex]?.id || null); window.scrollTo({ top: 0, behavior: "smooth" }); }}><BookOpen/><span>选题</span></button><button onClick={() => movePuzzle(1)}><ChevronRight/><span>下一题</span></button></>}
    ...
    {sheet && <BottomSheet title={sheetTitle} onClose={() => setSheet(null)}>
      ...
      {sheet === "export" && <div className="sheet-body export-options"><button onClick={() => { const exportDoc = hasDraft(draft) ? viewDocument : document; downloadText(exportSgf(exportDoc), `${safeName(exportDoc.metadata.title)}.sgf`, "application/x-go-sgf;charset=utf-8"); setToast("SGF 已导出"); }}><span className="format-icon">SGF</span><div><b>标准 SGF 棋谱</b><small>兼容变着、注释和棋盘标记</small></div><ArrowDownToLine/></button><button onClick={() => { const exportDoc = hasDraft(draft) ? viewDocument : document; downloadText(exportJson(exportDoc), `${safeName(exportDoc.metadata.title)}.renju`, "application/json;charset=utf-8"); setToast("跨端棋谱已导出"); }}><span className="format-icon json">R</span><div><b>RENJU 跨端文件</b><small>完整保留全部移动端数据</small></div><ArrowDownToLine/></button><button onClick={() => { const exportDoc = hasDraft(draft) ? viewDocument : document; downloadText(exportPos(exportDoc), `${safeName(exportDoc.metadata.title)}.pos.txt`, "text/plain;charset=utf-8"); setToast("主线坐标已导出（有损格式）"); }}><span className="format-icon">POS</span><div><b>主线坐标文本</b><small>仅导出默认主线；分支、注释和标注不会写入</small></div><ArrowDownToLine/></button><p className="helper">SGF 用于与现有五子棋软件交换；RENJU 文件完整保留本应用数据；POS/TXT 仅适合分享一条主线，属于有损转换。</p></div>}
      {sheet === "help" && <div className="sheet-body help-content"><div className="support-row"><b>棋谱导入</b><span>RenLib 3.x / 旧版无头 LIB（按设备能力分页导入）、SGF / FGF、REN / RENJS / WZQ（SGF 语法）、RENJU JSON、POS，以及 TXT 坐标序列。SGF 支持设置局面、过手、UTF-16 和同文件多盘棋。</span></div><div className="support-row"><b>JSON 的三种用途</b><span>棋谱库读取本软件的 RENJU 完整变化树或带明确 moves 字段的落子列表对象；题库页读取开宝兼容数组。数字坐标列表必须声明 coordinateBase，不猜测任意数组。</span></div><div className="support-row warning"><b>十五路边界</b><span>当前棋盘、规则与题库固定为十五路；其他 SGF SZ 会明确拒绝，不会缩放后生成错误棋谱。</span></div><div className="support-row warning"><b>TXT 不是统一棋谱标准</b><span>TXT 仅作为纯文本坐标序列兼容入口，例如 H8 I8 H9；带专有结构的文本应使用原软件导出的 SGF。</span></div><div className="support-row warning"><b>LIB 兼容边界</b><span>大型 LIB 在后台线程解析并按页存储。原谱的普通注释、局面文字和 RenLib 标记会分别保留并在节点详情中显示；超出 RenLib 3.4 的扩展仍会提示。LIB 不再设置固定的 200MB 文件上限，实际边界由设备内存和本地存储空间决定。</span></div><h3>手机快捷操作</h3><ul><li>点空交叉点：落子；点已有棋子：跳到该手</li><li>底部“标注”：放置数字、胜败平衡和自定义文字</li><li>长按交叉点：圆圈 → 三角 → 叉号 → 清除</li><li>左右方向键（外接键盘）：前后导航</li></ul><button className="primary-button" onClick={() => setSheet(null)}>知道了</button></div>}
      {sheet === "about" && <div className="sheet-body about-sheet"><section className="about-hero"><span>半</span><div><b>半步五子棋</b><small>版本 1.1.2 · 移动优先的打谱与做题工具</small></div></section><section className="creator-message"><b>写在前面</b><p>这是一个 Vibecoding 的产物，也是一款永久免费、开放源代码的五子棋软件。希望它能让手机打谱和做题更方便；如果内容涉及侵权，请通过 GitHub 联系，我会及时处理或删除。</p></section><section className="about-card"><h3><Code2 size={17}/>参考与致谢</h3><p>打谱功能参考了爱五子棋打谱软件与 RenLib / SGF 生态；做题交互和题集格式参考了开宝五子棋；AI 搜索思路参考了 SlowRenju 等公开项目。感谢这些前辈软件与开源社区。</p></section><section className="about-card"><h3><Layers3 size={17}/>技术架构</h3><p>React 19 + TypeScript + Vite · PWA / Workbox 离线网页 · Capacitor 8 Android · Web Worker 本地 AI 与 VCF 搜索。棋谱采用变化树模型，为网页、安卓和未来桌面端共享。</p></section><a className="github-link" href="https://github.com/gugujiao953-ship-it/banbu-gomoku" target="_blank" rel="noreferrer"><Code2 size={20}/><span><b>GitHub 源代码</b><small>gugujiao953-ship-it/banbu-gomoku</small></span><ChevronRight size={18}/></a><button className="primary-button" onClick={() => setSheet(null)}>完成</button></div>}
