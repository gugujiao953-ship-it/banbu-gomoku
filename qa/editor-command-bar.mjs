...
await context.addInitScript(() => {
  if (sessionStorage.getItem("__banbu_qa_cleaned") !== "1") {
    localStorage.clear();
    localStorage.setItem("banbu-first-run-welcome-v1", "true");
    sessionStorage.setItem("__banbu_qa_cleaned", "1");
  }
});
...
try {
  ...
  // T5 曾删「主动思考」按钮；T13 起「分析」按钮回归为功能区常驻入口（三模式
  // segment 的一部分）——断言存在而非不存在（2026-09-11 全功能回归纠正旧断言）。
  assert(await page.getByRole("button", { name: "分析" }).count() >= 1, "分析入口缺失");
  ...
  // 手机视口下 facts 区（.status-state）被 CSS 隐藏（≤699px absolute clip），
  // innerText 为空——改读状态栏全文（标题行 compat 文本含 stateLabel）。
  assert((await page.locator(".unified-status").innerText()).includes("未保存草稿"), "落子后未进入草稿状态");
  ...
  assert((await page.locator(".unified-status").innerText()).includes("未保存草稿"), "另存为题库错误清除了源棋谱草稿");
  ...
  const [sgfDownload] = await Promise.all([
    ...
    page.getByRole("button", { name: /SGF（整谱）/ }).click(),
  ]);
  ...
  const [jsonDownload] = await Promise.all([
    page.waitForEvent("download"),
    page.getByRole("button", { name: /JSON（整谱）/ }).click(),
  ]);
