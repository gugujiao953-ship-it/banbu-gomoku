...
- 本地棋谱库和约 0.5 秒防抖自动保存
- 棋谱库按棋谱名、棋手、赛事主题即时检索
- 最多 50 份棋谱批量导入；LIB 单文件最大 200MB，大文件由 Worker 线性解析并存入 IndexedDB，启动时只读摘要、点开后再载入完整变化树
...
`.lib` 是 RenLib 的二进制变化树格式。当前版本已支持现代 `RenLib 3.x` 头、旧版无头文件、主线/分支控制字节和常见节点注释；8MB 以上在后台线程解析，4MB / 4 万节点以上进入 IndexedDB 大型棋谱库。遇到扩展文本或超出 3.4 的版本会保留安全提示，不会静默损坏棋谱。
